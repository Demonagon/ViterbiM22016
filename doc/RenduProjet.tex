...
\begin{itemize}
\item La première ligne, bornée dans $[45.5\%, 60.7\%]$, qui représente les résultats sans lissage
\item Le reste du tableau, borné dans $[91.4\%, 94.4\%]$, qui représente les résultats avec lissage
\end{itemize}
\paragraph{}
Cette observation est une claire preuve que l'utilisation du lissage permet une grande précision de notre modèle génératif sur notre jeu de donnée.
\paragraph{}
...
\paragraph{}
Jetons maintenant un coup d'oeil sur la matrice contenant le temps de calcul des résultats obtenus dans la matrice précédente :\\
...
\paragraph{}
Devant analyse, ces résultats en temps du modèle génératif sont très constant. Le programme semble toujours s'exécuter autour des 3 secondes. Le paramètre $\alpha$ semble avoir une incidence ; lorsqu'il est égal à 0 ou 1, le temps global diminue de quelques centièmes de secondes. Cela s'explique vraisemblablement par l'optimisation de calcul du C lorsqu'il n'a pas à utiliser de nombres flottants.
\paragraph{}
...
\\
\paragraph{}
Devant ces résultats, il est correct de dire que le modèle génératif est un très bon modèle conçernant notre corpus. 6\% d'erreur est certes un très bon résultat, mais ce qui fait encore plus la force de ce modèle c'est sa grande rapidité de calcul ; cela est idéal lorsque l'on veut estimer la valeur optimal d'un paramètre, par exemple.

\subsection{Résultats via le modèle discriminant}\label{résultatsdiscriminant}

\paragraph{}
Dans le cadre du modèle discriminant, nous avons effectué des tests de nature très similaire. Simplement, au lieu de faire varier le paramètre $\alpha$ ici inutile, nous avons fait varier le paramètre $I$ qui représente le nombre d'itération de l'algorithme perceptron nécessaire au modèle disciminant. Il va avoir, comme nous le verrons, un grand impact autant sur le temps de calcul que la précision des résultats.\\
...
\paragraph{}
Ces quatre tableau offre dans leur ensemble une idée assez claire de la situation ; et il est juste de pouvoir dire que le modèle génératif est ici un meilleur modèle que le modèle discriminant. Les deux modèles semblent offrir dans les données un optimum similaire, autour des 5.7\% d'erreur. Cependant la comparaison en temps est, elle, ne porte à aucune hésitation, le modèle génératif restant remarquablement constant dans son temps d'exécution, là où le modèle discriminant prend très rapidement de longues minutes pour offrir un résultat similaire.

\subsection{Conclusion}\label{Conclusion}
Le problème de l'étiquettage est, dans la pratique, un problème complexe à aborder efficacement. En effet, et comme la plupart des problèmes qu'aborde les sciences du traitement automatique de la langue, c'est un problème qui repose sur des grandes masses de données ; et cela pose plusieurs contraintes. La première, c'est qu'il n'arrive jamais en pratique que cette donnée soit d'une forme pratique et utilisable directement. La seconde, c'est que la moindre complexité algorithmique prend des dimensions astromnomiques très rapidement.
\paragraph{}
...
\paragraph{}
On serait tenté de qualifier un algorithme comme le perceptron inefficace quant au problème d'étiquettage ; cependant, les choses sont un peu plus complexes. En effet, les corpus entièrement étiquetés sont dans la pratique extrêmement rares et les informaticiens doivent constamment jouer avec données dont ils ne savent pas tout. Bien souvent les modèles génératifs sont trop simplistes pour fonctionner, car trop exigeants sur la forme du corpus. \\
\paragraph{}
